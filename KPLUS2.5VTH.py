...
# ==============================================================================
# SECTION 3: PREDICTION LOGIC (UPGRADED TO 'LOGIC SIÊU CẤP')
# ==============================================================================
def choose_safe_room(recent_100, lose_streak=0, last_win_room=None, user_id=None, issue_id=None):
    """
    Logic Siêu Cấp: Tích hợp Chế Độ Phòng Thủ và các lớp phân tích sâu hơn.
    Mục tiêu: Tối đa hóa tỷ lệ thắng, chủ động ngắt chuỗi thua.
    """
    try:
        ...
        # Kích hoạt Chế Độ Phòng Thủ khi thua 2 ván liên tiếp
        ...
        # Logic mới: Loại trừ phòng vừa "gấp thếp" (ra 2 lần liên tiếp)
        ...
        # --- LỚP 2: PHÂN TÍCH RỦI RO ĐA CHIỀU (với các chuyên gia mới) ---
        risky_rooms = defaultdict(set)
        
        # Chuyên gia 1: Xu hướng nóng/lạnh
        ...
        # Chuyên gia 2: Phân tích chuyển tiếp (A -> B)
        ...
        # Chuyên gia 3: Phân tích vị trí & đối xứng
        ...
        # Chuyên gia 4 (Mới): Phân tích Ping-Pong (A-B-A)
        if len(full_history) > 2 and full_history[0] == full_history[2]:
            risky_rooms["ping_pong"].add(full_history[1])

        # Chuyên gia 5 (Mới): Phân tích Chẵn Lẻ
        recent_4_parity = [r % 2 for r in full_history[:4]]
        if len(set(recent_4_parity)) == 1: # Cả 4 ván gần nhất cùng chẵn hoặc cùng lẻ
            target_parity = 1 - recent_4_parity[0] # Nhắm đến chẵn/lẻ ngược lại
            for r in range(1, 9):
                if r % 2 != target_parity:
                    risky_rooms["parity"].add(r)


        # --- LỚP 3: LỰA CHỌN THEO BẬC ƯU TIÊN (Tích hợp Chế Độ Phòng Thủ) ---
        ...
        if DEFENSIVE_MODE:
            if not tier1_safe_pool:
                return None, "Bỏ qua (Phòng thủ)" # Tín hiệu bỏ qua vòng cược
...
def main():
    ...
    # SỬA LỖI: Thêm 'attempted_bet_for_issue' để chống cược nhiều lần
    ...
    while True:
        try:
            ...
            if latest_issue_id != session_state["last_known_issue_id"]:
                ...
                # THÊM MỚI: Bỏ qua 2 ván đầu tiên
                if session_state['round'] <= 2:
                    ...
                    time.sleep(3) # Dừng một chút để người dùng đọc thông báo
                    continue

                # SỬA LỖI: Đảm bảo chỉ có một hành động (cược/bỏ qua) cho mỗi ván mới
                if session_state['attempted_bet_for_issue'] == next_round_id:
                    time.sleep(3) # Nếu đã xử lý ván này rồi, kiên nhẫn chờ kết quả
                    continue
                # Đánh dấu ván này sẽ được xử lý để không bị lặp lại
                ...
                # SỬA LỖI: Tăng delay cược để ổn định hơn
